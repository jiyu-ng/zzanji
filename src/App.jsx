...
const EXPENSE_CATS = ["식비", "카페", "교통", "쇼핑", "생활", "의료", "문화", "기타"];
const INCOME_CATS = ["월급", "용돈", "부수입", "기타"];
const CAT_EMOJI = {
  식비: "🍚", 카페: "☕", 교통: "🚕", 쇼핑: "🛍️", 생활: "🏠", 의료: "💊", 문화: "🎬",
  월급: "💼", 용돈: "💵", 부수입: "💰", 기타: "📦",
};
const CAT_COLOR = ["#e8865a", "#5aa7e8", "#63c187", "#c77dd6", "#e0b64a", "#e8724a", "#7d8fd6", "#9aa0a6"];
...
export default function App() {
  ...
  const [form, setForm] = useState({ date: todayStr(), type: "expense", amount: "", category: "식비", item: "" });
  ...
  const submit = async () => {
    ...
    setSaving(true);
    const row = { date: form.date, type: form.type, amount: amt, category: form.category, item: form.item.trim() || null };
  ...
  return (
    <div style={wrap}>
      <header style={head}>
        ...
        <h1 style={h1}>짠지</h1>
        <p style={{ margin: "3px 0 0", color: "#b3a99a", fontSize: 12.5 }}>짠순이 지유의 가계부 🐷💰</p>
      </header>
...
const head = { textAlign: "center", padding: "24px 0 8px" };
const h1 = { fontSize: 22, fontWeight: 800, margin: "4px 0 0" };
const monthBar = { display: "flex", alignItems: "center", justifyContent: "center", gap: 18, margin: "8px 0 16px" };
...
const sheet = { width: "100%", maxWidth: 520, background: "#faf7f2", borderRadius: "22px 22px 0 0", padding: "22px 18px calc(24px + env(safe-area-inset-bottom))" };
